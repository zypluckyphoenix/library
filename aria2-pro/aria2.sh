...
aria2c="${aria2_conf_dir}/aria2c"
GHPROXY=https://ghproxy.cn/
...
check_installed_status() {
  [[ ! -f ${aria2c} ]] && echo -e "${Error} Aria2 没有安装，请检查 !" && exit 1
  [[ ! -f ${aria2_conf} ]] && echo -e "${Error} Aria2 配置文件不存在，请检查 !" && [[ $1 != "un" ]] && exit 1
}
...
check_new_ver() {
  ...
  if [[ -z ${aria2_new_ver} ]]; then
    ...
    read -e -p -r "请输入版本号:" aria2_new_ver
    [[ -z "${aria2_new_ver}" ]] && echo "取消..." && exit 1
  fi
}
check_ver_comparison() {
  read -e -p -r "是否更新(会中断当前下载任务) ? [Y/n] :" yn
  [[ -z "${yn}" ]] && yn="y"
  if [[ $yn == [Yy] ]]; then
    check_pid
    [[ -n $PID ]] && kill -9 "${PID}"
...
Download_aria2() {
  ...
  while which aria2c >/dev/null; do
  ...
  {
    wget -t2 -T3 -O- "${DOWNLOAD_URL}" || wget -t2 -T3 -O- "https://gh-acc.p3terx.com/${DOWNLOAD_URL}" || wget -t2 -T3 -O- "${GHPROXY}${DOWNLOAD_URL}"
  } | tar -zx
...
Download_aria2_conf() {
  PROFILE_URL1="https://github.com/P3TERX/aria2.conf/raw/master/"
  PROFILE_URL2="https://gh-acc.p3terx.com/https://github.com/P3TERX/aria2.conf/raw/master/"
  PROFILE_URL3="https://fastly.jsdelivr.net/gh/P3TERX/aria2.conf@master/"
  PROFILE_LIST="
aria2.conf
clean.sh
core
...
"
  ...
  for PROFILE in ${PROFILE_LIST}; do
    ...
    wget -N -t2 -T3 "${PROFILE_URL1}/${PROFILE}" || wget -N -t2 -T3 "${PROFILE_URL2}/${PROFILE}" || wget -N -t2 -T3 "${PROFILE_URL3}/${PROFILE}"
    [[ ! -s ${PROFILE} ]] && {
...
Service_aria2() {
  if [[ ${release} = "centos" ]]; then
    wget -N -t2 -T3 "https://github.com/P3TERX/aria2.sh/raw/master/service/aria2_centos" -O /etc/init.d/aria2 ||
      wget -N -t2 -T3 "https://gh-raw.p3terx.com/P3TERX/aria2.sh/master/service/aria2_centos" -O /etc/init.d/aria2 ||
      wget -N -t2 -T3 "https://fastly.jsdelivr.net/gh/P3TERX/aria2.sh@master/service/aria2_centos" -O /etc/init.d/aria2
    [[ ! -s /etc/init.d/aria2 ]] && {
    ...
    wget -N -t2 -T3 "https://github.com/P3TERX/aria2.sh/raw/master/service/aria2_debian" -O /etc/init.d/aria2 ||
      wget -N -t2 -T3 "https://gh-raw.p3terx.com/P3TERX/aria2.sh/master/service/aria2_debian" -O /etc/init.d/aria2 ||
      wget -N -t2 -T3 "https://fastly.jsdelivr.net/gh/P3TERX/aria2.sh@master/service/aria2_debian" -O /etc/init.d/aria2
    [[ ! -s /etc/init.d/aria2 ]] && {
      echo -e "${Error} Aria2服务 管理脚本下载失败 !"
      exit 1
    }
...
"
  read -e -p -r " 请输入数字 [0-5]:" aria2_modify
...
"
  read -e -p -r " 请输入新的 RPC 密钥: " aria2_RPC_passwd
  echo
  ...
  if [[ "${aria2_passwd}" != "${aria2_RPC_passwd}" ]]; then
    if [[ -z "${aria2_passwd}" ]]; then
      if echo -e "\nrpc-secret=${aria2_RPC_passwd}" >>${aria2_conf}; then
        ...
        if [[ ${read_123} != "1" ]]; then Restart_aria2; fi
      else echo -e "${Error} RPC 密钥修改失败！旧密钥为：${Green_font_prefix}${aria2_passwd}${Font_color_suffix}"; fi
    else
  ...
  else echo -e "${Error} 与旧配置一致，无需修改..."; fi
}
Set_aria2_RPC_port() {
  read_123=$1
  if [[ ${read_123} != "1" ]]; then
...
"
  read -e -p -r " 请输入新的 RPC 端口(默认: 6800): " aria2_RPC_port
  echo
  ...
  if [[ "${aria2_port}" != "${aria2_RPC_port}" ]]; then
    if [[ -z "${aria2_port}" ]]; then
      if echo -e "\nrpc-listen-port=${aria2_RPC_port}" >>${aria2_conf}; then
        ...
        # Del_iptables; Add_iptables; Save_iptables
        if [[ ${read_123} != "1" ]]; then Restart_aria2; fi
      else echo -e "${Error} RPC 端口修改失败！旧端口为：${Green_font_prefix}${aria2_port}${Font_color_suffix}"; fi
    else
      # shellcheck disable=SC2086
...
Set_aria2_RPC_dir() {
  read_123=$1
  if [[ ${read_123} != "1" ]]; then Read_config; fi
  if [[ -z "${aria2_dir}" ]]; then
    aria2_dir_1="空(没有检测到配置，可能手动删除或注释了)"
  else aria2_dir_1=${aria2_dir}; fi
  echo -e "
 当前下载目录为: ${Green_font_prefix}${aria2_dir_1}${Font_color_suffix}
"
  read -e -p -r " 请输入新的下载目录(默认: ${download_path}): " aria2_RPC_dir
  ...
  if [[ "${aria2_dir}" != "${aria2_RPC_dir}" ]]; then
    if [[ -z "${aria2_dir}" ]]; then
      if echo -e "\ndir=${aria2_RPC_dir}" >>${aria2_conf}; then
        ...
        if [[ ${read_123} != "1" ]]; then Restart_aria2; fi
      else echo -e "${Error} 下载目录修改失败！旧位置为：${Green_font_prefix}${aria2_dir}${Font_color_suffix}"; fi
    else
      aria2_dir_2=$(echo "${aria2_dir}" | sed 's/\//\\\//g')
      aria2_RPC_dir_2=$(echo "${aria2_RPC_dir}" | sed 's/\//\\\//g')
      sed -i "s@^\(dir=\).*@\1${aria2_RPC_dir_2}@" ${aria2_conf}
      if sed -i "s@^\(DOWNLOAD_PATH='\).*@\1${aria2_RPC_dir_2}'@" ${aria2_conf_dir}/*.sh; then
...
Set_aria2_vim_conf() {
 ...
 "
  read -e -p -r "按任意键继续，按 Ctrl+C 组合键取消" var
  nano "${aria2_conf}"
  Read_config
...
View_Aria2() {
  check_installed_status
  Read_config
  IPV4=$(wget -qO- -t1 -T2 -4 api.ip.sb/ip || wget -qO- -t1 -T2 -4 ifconfig.io/ip || wget -qO- -t1 -T2 -4 www.trackip.net/ip)
  IPV6=$(wget -qO- -t1 -T2 -6 api.ip.sb/ip || wget -qO- -t1 -T2 -6 ifconfig.io/ip || wget -qO- -t1 -T2 -6 www.trackip.net/ip)
...
Update_bt_tracker_cron() {
  ...
  if [[ -z $(crontab_update_status) ]]; then
    ...
    read -e -r crontab_update_status_ny
...
Update_Shell() {
  sh_new_ver=$(wget -qO- -t1 -T3 "https://gh-acc.p3terx.com/https://github.com/P3TERX/aria2.sh/raw/master/aria2.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1) && sh_new_type="github"
  ...
  {
    wget -N -t2 -T3 "https://github.com/P3TERX/aria2.sh/raw/master/aria2.sh" -O aria2.sh ||
      wget -N -t2 -T3 "https://gh-raw.p3terx.com/P3TERX/aria2.sh/master/aria2.sh" -O aria2.sh ||
      wget -N -t2 -T3 "https://fastly.jsdelivr.net/gh/P3TERX/aria2.sh@master/aria2.sh" -O aria2.sh
  } && chmod +x aria2.sh
